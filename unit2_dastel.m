Fs = 200;
t = [0:3*Fs+1]'/Fs; Fc = 10;
x = sin(2*pi*t); subplot(3,1,1);
plot(t,x); hold on;
%AM-DSB-SC
ydouble = modulate(x,Fc,Fs,'amdsb-sc'); plot(t,ydouble,'r')
...
subplot(3,1,2);
plot(t,y2); xlabel('waktu t (detik)');
ylabel('demodulator DSB-FC'); axis([0 3 -2 2]);
grid on;
%AM-SSB
y3 = demod(ysingle,Fc,Fs,'amssb');
subplot(3,1,3);
plot(t,y3); xlabel('waktu t (detik)');
ylabel('demodulator SSB'); axis([0 3 -1 1]);
grid on;
